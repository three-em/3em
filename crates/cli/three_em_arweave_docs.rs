...
/**
 * @Purpose - Hold a lazy initiated value representing Arweave Cache
 * 
 * @Note
 * `OnceCell` - Lazy initiates values, good for performance
 * `Arc` - Smart pointer allowing data to be shared between threads
 * `Mutex` - Allows one user of the cache at a time.
 * 
 * `dyn` - dynamic trait object - will take any type as long as it executes the other traits
 * `CacheExt` - trait w/ function signatures on contract & cache manipulation
 * `Send` - trait allowing data to be sent between threads i.e. transfer of ownership of data
 * `Sync` - data can be shared between threads, no transfer of ownership in data
 * 
 * @NOTE - CacheExt trait included in bottom of file.
 */
