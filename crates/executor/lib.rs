...
mod test {
  ...
  async fn test_sorting_all() {
    ...
    let smartweave_validity: IndexMap<String, bool> = serde_json::from_str(smartweave).unwrap();
    let arweave =
    ...
    if let ExecuteResult::V8(value, validity) = result {
      let map: IndexMap<String, bool> = validity;
      let keys = map.keys();
      ...
      for i in 0..(keys.len()) {
        let (key, value): (&String, &bool) = map.get_index(i).unwrap();
        let (smartweave_key, smartweave_value): (&String, &bool) = smartweave_validity.get_index(i).unwrap();

        if smartweave_key != key {
          panic!("Key index #{} from 3EM {} is different from smartweave {}", i, key, smartweave_key);
        }
