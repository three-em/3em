...
pub fn get_core_nodes() -> Vec<Node> {
    ...
    let nodes: Vec<String> = nodes.iter().filter(|&p| !(p.eq(""))).cloned().collect();
    let result = nodes.iter().map(|content| {
        let data: Vec<String> = content.split(":").map(|node| String::from(node)).collect();
